package es_uclm_esi_isoft2_b03_restaurante_pedidoComandas_persistencia;
...
public class AlimentoDAO {
	...
	public static Alimento stringToAlimento(String r) {
		...
		try {
			int idPlato = Integer.parseInt(st.nextToken().replace("\"", ""));
			String nombre = Integer.parseInt(st.nextToken().replace("\"", ""));
			float cantidadIngrediente = Float.parseFloat(st.nextToken().replace("\"", ""));

			alimento = new Ingrediente(idIngrediente, cantidadIngrediente);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return alimento;
